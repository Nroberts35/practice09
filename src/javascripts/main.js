...
function searchMovies(event){
    ...
    let input = document.querySelector('[type="search"]').value || "" // get the input or an empty string
    let count = 0
    for(let m of movies) {
        if(m.title.toUpperCase().indexOf(input.toUpperCase()) == -1){ // if the movie doesn't exist, set its display to none
            document.querySelector(`#m${m.id}`).classList.add('d-none')
        }
        else {
            document.querySelector(`#m${m.id}`).classList.remove('d-none') // if it does exist, display it.
            count++ //increment count if a movie was found
        }
    }
